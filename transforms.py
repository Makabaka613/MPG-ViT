...
class RandomCopyMove(DualTransform):   #RandomCopyMove 类是一个用于在图像上执行“复制-移动”操作的数据增强模块，同时在掩膜上更新对应的区域。该类继承自 DualTransform
    def __init__(self,
        max_h = 0.8,
        max_w = 0.8,
        min_h = 0.05,
        ...
        super(RandomCopyMove, self).__init__(always_apply=False, p=0.5)
        self.max_h = max_h
        self.max_w = max_w
        self.min_h = min_h
        self.min_w = min_w
    ...
    def _get_random_window(
        ...
        return pos_h, pos_w , window_h, window_w   #计算最小和最大窗口的高度和宽度。随机生成窗口的大小和位置。
        
        
    def apply(self, img: np.ndarray, **params) -> np.ndarray:
        image = img.copy()    #创建图像的副本，并获取图像的高度和宽度。
        H, W, _ = image.shape
        # copy region:
        c_pos_h, c_pos_w, c_window_h, c_window_w = self._get_random_window(H, W)   #使用 _get_random_window 方法获取复制区域和放置区域的随机位置和大小。
        ...
        copy_region = image[               #将选中的区域 copy_region 复制到新的位置。
...
class RandomInpainting(DualTransform):   #RandomInpainting 类是一个用于图像数据增强的变换类，它通过随机选择图像区域进行修补（inpainting），并更新相应的掩膜（mask）。这个类继承自 DualTransform，通常用于图像处理框架（如 albumentations）中
    def __init__(self,
        max_h = 0.8,
        max_w = 0.8,
        min_h = 0.05,
        ...
        super(RandomInpainting, self).__init__(always_apply=False, p=0.5)
        self.max_h = max_h
        self.max_w = max_w
        self.min_h = min_h
        self.min_w = min_w
...
def get_albu_transforms(type_ = 'train', outputsize = 1024):   #get_albu_transforms 函数用于根据不同的任务（训练、测试或填充）返回相应的图像增强变换。这个函数使用了 albumentations 库来定义变换操作，并根据参数 type_ 的值应用不同的变换策略。
