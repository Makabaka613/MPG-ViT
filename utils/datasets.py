...
def text_loader(key_name, mode):
    if mode == "train":
        pkl_path = '/root/autodl-tmp/vit/text_tensor/tensorcasiav2/image_features.pkl'

    elif mode == "test":
        pkl_path = '/root/autodl-tmp/vit/text_tensor/tensornist16/image_features.pkl'
    else:
        print(f"mode = {mode}")
    ...
    # print(f"Sample keys from the image features dictionary: {list(image_features_dict.keys())[:5]}")

    # 检查键是否存在
    if key_name in image_features_dict:
        value = image_features_dict[key_name]
        tensor = torch.tensor(value)  # 转换为 PyTorch 张量
        return tensor
    else:
        print(f"pkl_path={pkl_path}")
        print(f"mode={mode}")
        raise KeyError(f"Key '{key_name}' not found in the image features dictionary.")
...
class base_dataset(Dataset):
    ...
    def __init__(self, path, mode,output_size = 1024 ,transform = None, edge_width = None, if_return_name = False, if_return_shape = False, if_return_type = False,text_tensor = None) -> None:
    # def __init__(self, path, output_size = 1024 ,transform = None, edge_width = None, if_return_name = True, if_return_shape = False, if_return_type = False) -> None:
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        file_name_without_extension = os.path.splitext(os.path.basename(self.tp_path[index]))[0]

      
        text_tensor = text_loader(file_name_without_extension, mode = self.mode)
        # print("dataloader中 text_tensor",text_tensor.shape)      # dataloader中 text_tensor torch.Size([1, 1024])
        ...
        assert tp_shape == gt_shape, "tp and gt image shape must be the same, but got {} and {},name = {},{},{}".format(tp_shape, gt_shape,tp_path,gt_path,index)
        ...
        # print('**********************************************',tp_img.shape,gt_img.shape)          # (490, 653, 3) (490, 653, 3)
        ...
        output_list.append(text_tensor)
...
if __name__ == "__main__":
    ...
    data = mani_dataset(r'/root/data1/IML-ViT-main/Dataset/CASIA2.0', edge_width=5, transform=transform, if_return_shape=True)
    ...
    for sample in d:
        ...
        image, mask, b_mask, text_tensor, shape = sample
