...
def main(args):
    ...
    if os.path.isdir(args.data_path):
        dataset_train = utils.datasets.mani_dataset(args.data_path, transform=train_transform, edge_width=args.edge_broaden, if_return_shape=True, mode = 'train')
    else:
        dataset_train = utils.datasets.json_dataset(args.data_path,transform=train_transform, edge_width = args.edge_broaden, if_return_shape = True, mode = 'train')
    
    if os.path.isdir(args.test_data_path):
        dataset_test = utils.datasets.mani_dataset(args.test_data_path, transform=test_transform, edge_width=args.edge_broaden, if_return_shape=True, mode = 'test')
    else:
        dataset_test = utils.datasets.json_dataset(args.test_data_path,transform=test_transform, edge_width = args.edge_broaden, if_return_shape = True, mode = 'test')
...
#######################################################################################################################
    # for name, para in model_without_ddp.encoder_net.ViT:
    #     # 除head, pre_logits外，其他权重全部冻结
    #     if "prompt_blocks" not in name:
    ...
    # # for p in model_without_ddp.encoder_net.patch_embed.parameters():
    # print('*****************************************************************************************************************')
    # print(model_without_ddp.encoder_net.prompt_blocks[0])
    # print('*****************************************************************************************************************')
    # for p in model_without_ddp.encoder_net.prompt_blocks[0].parameters():
...
###############################################################################################################################
    ...
    loss_scaler = NativeScaler(growth_factor=2, growth_interval=100)
